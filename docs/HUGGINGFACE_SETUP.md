...
**Hugging Face Spaces** is the recommended free host: it gives you **2 vCPU +
16 GB RAM** (plenty for PyTorch), runs the Docker image, and serves a permanent
public URL like `https://your-username-milerunner.hf.space`. Training starts
automatically and the live dashboard updates in real time.
...
3. **License**: MIT. **Select the SDK**: choose **Docker** → **Blank**.
4. **Hardware**: leave the free **CPU basic** (2 vCPU · 16 GB RAM).
5. Set it **Public** and click **Create Space**. It's empty for now.
...
Run these in a terminal, replacing `YOUR_USERNAME` (and paste the token when git
asks for a password):
...
# use the Hugging Face front-matter README (tells HF to build the Dockerfile)
...
- The **Building** logs run for a few minutes (installing PyTorch etc.).
...
1. Create the Space and an HF **write** token (steps 1–2 above).
...
3. Go to the repo's **Actions** tab → run **Deploy to Hugging Face Spaces**
   ...
   Space and it builds automatically from then on.

---

## Tips & troubleshooting

- **"Waiting for data…" on first load** — normal; the trainer hasn't finished
...
- **Make it train harder** — edit `configs/hosted.yaml` (raise `population.size`
  ...
  headroom.
- **Build fails on memory during pip** — rare on Spaces; if so, keep the CPU
  torch line as-is (it's the small wheel) and retry the build from the Space's
